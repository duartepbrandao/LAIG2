			...
			for( ; itg != m_Groups.end(); ++itg ) {
				...
				if(itg->m_AssignedMaterials.size())	{
					...
					for( ; itmg != itg->m_AssignedMaterials.end(); ++itmg ) {
						...
						if(m_Materials[itmg->m_MaterialIdx].map_Kd!=""){
							cout<<"\n\n\n\n "<<m_Materials[itmg->m_MaterialIdx].map_Kd<<"\n\n\n\n"<<endl;
							CGFappearance* apps=new CGFappearance(m_Materials[itmg->m_MaterialIdx].map_Kd, GL_CLAMP, GL_CLAMP);
							apps->setAmbient(m_Materials[itmg->m_MaterialIdx].Ka);
							apps->setDiffuse(m_Materials[itmg->m_MaterialIdx].Kd);
							apps->setSpecular(m_Materials[itmg->m_MaterialIdx].Ks);
							apps->setShininess(m_Materials[itmg->m_MaterialIdx].Ns);
							apps->apply();
						}
						if(m_Materials[itmg->m_MaterialIdx].map_Ka!=""){
							cout<<"Ola_1"<<endl;
							CGFappearance* apps=new CGFappearance(m_Materials[itmg->m_MaterialIdx].map_Ka, GL_CLAMP, GL_CLAMP);
							apps->apply();
						}
						if(m_Materials[itmg->m_MaterialIdx].map_Ks!=""){
							cout<<"Ola_2"<<endl;
							CGFappearance* apps=new CGFappearance(m_Materials[itmg->m_MaterialIdx].map_Ks, GL_CLAMP, GL_CLAMP);
							apps->apply();
						}
						if(m_Materials[itmg->m_MaterialIdx].map_Bump!=""){
							cout<<"Ola_3"<<endl;
							CGFappearance* apps=new CGFappearance(m_Materials[itmg->m_MaterialIdx].map_Bump, GL_CLAMP, GL_CLAMP);
							apps->apply();
						}
